...
var Resource = {
	...
	columns :  [[
		           {field: 'resourceName', title:'资源名称' , width: '20%' , align: 'left'},
		           {field: 'module', title:'资源模块' , width: '17%' , align: 'center'},
		           {field: 'actionCode', title:'资源代码' , width: '10%' , align: 'center'},
		           {field: 'resourceUrl', title:'资源路径' , width: '30%' , align: 'left'},
		           {field: 'category', title:'资源分类' , width: '10%' , align: 'center'},
		           {field: 'resourceType', title:'访问类别' , width: '10%' , align: 'center'}
		           ]],
	menuId: 'Resource',
...
Resource.Menu = {
		config: {
	        ...
	        autoRowHeight: false,
	        columns:[[
	            {title:'菜单节点名称',field:'text',width:250, editor:'text'},
	            {title:'排序',field:'menuOrder',width:30},
	            {title:'菜单范围路径',field:'resourceUrl',width:450,align:'left'}
	        ]],
	        onContextMenu: function(e,row){
	            ...
	            if (row){
	                ...
	                if(parentRow){
		                var allChildrenNodes  = $('#menutree').treegrid('getChildren', parentRow.id); 
		                var brotherNodes = Resource.Menu.getBrotherNodes(allChildrenNodes, level);
			...
			onClickRow: function(row){
				Resource.Menu.currentRow = row;
				Resource.Menu.parentRow =  $('#menutree').treegrid('getParent', row.id); 
			},
			onLoadSuccess : function(node,data){
				if(initLoad==0){
					$('.datagrid-row').attr('height','40px');
					for(var i =0 ; data &&  data.rows && i < data.rows.length; i++){
						$('#menutree').treegrid('collapseAll', $('#menutree').treegrid("find",data.rows[i]["id"]).target); 
					}
				}
		...
		loadMenu: function(){
			CMC.request({url: Resource.loadMenuUrl ,
					method: 'GET',
		...
		deleteMenu: function(){
			...
			CMC.request({
				...
				success : function(data){
					...
					var parentRow =  $('#menutree').treegrid('getParent', row.id); 
					var childRows =  $('#menutree').treegrid('getChildren', parentRow.id); 
					var brotherNodes = Resource.Menu.getBrotherNodes(childRows,row['level']);
					var submitRows = [];
		...
		selectMenu: function(){
			...
			var chlidrenNodes  = $('#menutree').treegrid('getChildren', row.id); 
			var brotherNodes = Resource.Menu.getBrotherNodes(chlidrenNodes,row['level'] + 1);
			var submitData  = {
                    ...
                    menuOrder : (brotherNodes==null||brotherNodes.length==0) ? 1 : (brotherNodes.length+1)
            };
...
(function($){
	...
	Resource.searchMenuResource = function(){
		CMC.request({
			...
			method: 'POST',
			success: function(response){
				$("#memuSearchTable").datagrid("loadData",response.messageBody.list);
	...
	Resource.init = function(){
		...
		$("#memuSearchTable").datagrid({
			...
			columns :  [[
				           ...
				           {field: 'resourceUrl', title:'资源路径' , width: "70%"  , align: 'center'}
				           ]]
		});
