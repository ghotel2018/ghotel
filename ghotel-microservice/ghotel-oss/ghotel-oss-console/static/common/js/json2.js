...
function showSeqAndCabin(depcity_code,arrcity_code){
	...
	if(flag){
		...
		//zhangchunping@foreveross.com
		var requerying_arrow = $("#arrow_requerying");
		var flightSelected_arrow = $("#arrow_flightSelected");
		//国际页面除外
		if (requerying_arrow.length > 0 && flightSelected_arrow.length > 0) {
			locatorDcselectedBox();	//因为会多一个排序类型层，所有要重新确认已选航班面板的位置
		}
	}else{
		$seqAndCabin.hide();
		$cabinSelect.hide();
...
function locatorDcselectedBox() {

	var cls = $("#arrow_requerying").attr("class");
	var _scrollTop = $("#researchfilght").offset().top + $("#researchfilght").height();
	if (cls == "arrow-up-icon") {
		_scrollTop += 20;
	} else if (cls == "arrow-down-icon") {
		_scrollTop += 15;
	}
	$("#dc_selectedBox").css({
		"top": _scrollTop
