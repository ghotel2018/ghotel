...
var PayBeforePnrManage={
    ...
    addUrl: 'authorized/payBeforeAsyncPnrManage/add',
    updateUrl: 'authorized/payBeforeAsyncPnrManage/update',
    deleteUrl: 'authorized/payBeforeAsyncPnrManage/delete',
    searchUrl: "authorized/payBeforeAsyncPnrManage/getAll" ,
    getSwitchUrl: "authorized/payBeforeAsyncPnrManage/getSwitch",
    changeSwitchUrl:"authorized/payBeforeAsyncPnrManage/changeSwitch",
    importUrl:"authorized/payBeforeAsyncPnrManage/import",
    columns:[[
        {field: 'cabinname', title:'舱位' , width: '30%' , align: 'center'},
        {field: 'flightline', title:'航线' , width: '16%' , align: 'center'},
        {field: 'flightdaterange', title:'航班日期' , width: '30%' , align: 'center'},
        {field: 'deptimelimited', title:'起飞时间限制' , width: '8%' , align: 'center'},
        {field: 'seatnum', title:'剩余座位数' , width: '8%' , align: 'center'},
        {field: 'paylimited', title:'支付时限（秒）' , width: '8%' , align: 'center'},
...
(function($){
    ...
    $("#payBeforePnrManage_add").click(function(event) {
        $('#addDetail').dialog({    
            ...
            resizable:true,    
            cache: false,    
            href: '/cmc/module/payBeforePnrManage/addAsyncCabinInfo.html',    
            modal: true,
            onLoad:function(){
    ...
    $("#payBeforePnrManage_update").click(function(event) {
        ...
        if(!code){
         ...
         $('#editDetail').dialog({    
            ...
            cache: false, 
            queryParams: code,   
            href: '/cmc/module/payBeforePnrManage/editAsyncPage.html',    
            modal: true,
            onLoad:function(){
...
$("#payBeforePnrManage_Template").click(function(event) {
    window.open(encodeURI("/cmc/download/payBeforeAsyncPnrManageTemplate.xlsx"));
});
...
$("#payBeforePnrManageInfo_import").click(function(event) {
    ...
    CMC.confirm("是否确认导入文件?",function(r){
        if(r){
            CMC.showProcessBox();
            CMC.fileUpload({
                url: PayBeforePnrManage.importUrl+".json",
                type: "POST",
                dataType: "json",
                fileElementId:  "payBeforeAsyncPnrManageInfoFile",
                // data: $("#cityListImportForm").form().serialize(),payBeforeAsyncPnrManageInfoFile
                asyc: true,
                timeout: 600000,
