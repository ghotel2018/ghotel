...
var CouponGroupMnt = {
	scope:'Singapore',
	menuId: "CouponGroupSingapore",
	searchTableRequired: true,
	initNotSearch:false,
	getConfigUrl: "authorized/couponGroupSingapore/pageConfig" ,
	...
	issueUrl: "authorized/couponGroupSingapore/issue" ,
	exportKeyUrl: "authorized/couponGroupSingapore/exportPrivateKey" ,
	exportReportUrl: "authorized/couponGroupSingapore/exportReport",
	getMailSentCountUrl: "authorized/couponGroupSingapore/getMailSentCount",
	updateMailSentCountUrl: "authorized/couponGroupSingapore/updateMailSentCount",
	searchUrl: "authorized/couponGroupSingapore/getSingaporeAll" , //如果searchTableRequired 是 true 必填
	searchSingaporeUrl: "authorized/couponGroupSingapore/getSingaporeAll" , //如果searchTableRequired 是 true 必填
	columns :  [[
		...
		{field: 'discountType', title:'优惠券类型' , width: '20%' , align: 'center',
			formatter:function(value,row,index){
				   var discountType="";
				   if(row.discountType==11||row.discountType=='15'){
					   discountType="新加坡自有资金优惠券";
				   }else if(row.discountType==12||row.discountType=='16'){
					   discountType="新加坡虚拟资金优惠券";
				   }
				   return discountType;
			   }
		},
...
(function($){
	...
	$('#searchFormAddSingapore').click(function(){
		...
		$("#createDialog").dialog({
			...
			cache:false,
			modal:true,
			href:"/cmc/module/coupon/couponGroupCreateSingaporeForm.html?scope=Singapore",
			onLoad:function () {

				$("#createDialog #isInternations_create").val("5");
				$("#createDialog #addBtn_create_domestic").hide();
				$("#createDialog #addBtn_create_I18n").hide();
				$("#createDialog #addBtn_create_Singapore").show();
				...
				CouponGroupCommon.intConfig(true,true,true,'Singapore');
			}

		});
		$('.error').html('');
		$("#count").html(500);
		$('#segmentType_create_Singapore').combobox('setValue','0');
		$("#createSingaporeCouponGroupForm input[name='flightTimeType']").each(function(j,el2){
			$(el2).prop("checked", true).attr("checked", true);
		});

		$("#createSingaporeCouponGroupForm .easyui-datebox").each(function(j,el2){
			$(el2).datebox().datebox('calendar').calendar({
				validator: function(date){
					var now = new Date();
					var d1 = new Date(now.getFullYear(), now.getMonth(), now.getDate());
	...
	$('#searchFormUpdateSingapore').click(function(){
		var record = CMC.grid.datagrid("getSelected");
		if(!record || record['groupId']==null || record['groupId']==''){
			CMC.alertMessage("请选择一条新加坡优惠券批次记录!", 'warn');
			return;
		}
		var groupId = record['groupId'];
		var src = "/cmc/module/coupon/couponGroupUpdateSingaporeForm.html?scope=Singapore&groupId="+groupId;
		//CouponGroupMnt.intConfig(false,false,true);

		$('.error').html('');
		$('#updateSingaporeCouponGroupForm').form('clear');
	...
	$("#searchFormIssue").click(function(){
		...
		if($("#issueFormDialog #container").length<=0){
			...
			$('#issueFormDialog').show().dialog({
				...
				cache: true,
				href:"/cmc/module/coupon/couponIssuSingaporeForm.html"
			});
		}else{
...
//		CMC.dialog('winMarketingActivityEffect','open');
	...
	$("#CouponGroup_Template").click(function(){
		window.open(encodeURI("/cmc/download/couponGroupSingaporeTemplate.xlsx"));
	});
	...
	$("#CouponGroupInfo_import").click(function(){
		...
		//审批人
		var approveUserName=$("#SingaporeAproveName").val();
		if(approveUserName==""){
		...
		var data={"aproveId": $("#SingaporeAproveId").val()};
		CMC.confirm("是否确认导入文件?",function(r){
			if(r){
				 CMC.showProcessBox();
				CMC.fileUpload({
					url: "authorized/couponGroupSingapore/importReport",
					type: "POST",
					dataType: "json",
...
function doSingaporeUserOneSearch() {
	openSelect();
	$("#selectApporUser").show().dialog("open");
	window.frames["apporUserSelect"].openIframeId="selectApporUser";
	window.frames["apporUserSelect"].userId="SingaporeAproveId";
	window.frames["apporUserSelect"].userName="SingaporeAproveName";
}
