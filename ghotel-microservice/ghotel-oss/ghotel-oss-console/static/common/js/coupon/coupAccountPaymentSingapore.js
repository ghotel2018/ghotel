...
var CmcCoupAccountPayment = {
	menuId: "CmcCoupAccountPaymentSingapore",
    searchUrl : '/authorized/coupAccountPaymentSingapore/getSingaporeAll',
    getSingaporeAccount : '/authorized/coupAccountPaymentSingapore/getSingaporeAccount',
    addUrl : '/authorized/coupAccountPaymentSingapore/addSingaporeAccount',
    exportedData:null,
...
CmcCoupAccountPayment.queryAccount = function(formid){
	CMC.request({
		url: CmcCoupAccountPayment.getSingaporeAccount,
		method: "get",
		dataType: "json",
		async: true,
...
CmcCoupAccountPayment.add=function(){
	$("#coupAccountPaymentSingaporeCreatFrom").dialog({
	    collapsible: true,
	    title: "虚拟帐号充值",
	    minimizable: false, 
	    maximizable: false,
	    ...
	    width: 400,
	    height: 350,
	    href:"/cmc/module/coupon/virtualAccountSingapore.html",
	    onLoad: function () {
	    ...
	    buttons:[{ 
			...
			handler:function(){ 
				...
				if(ok){
					...
					var remark=$("#addCoupAccountForm #remark").val();
					if(parseInt(valueNum)%5 != 0 || valueNum.length<2 ||valueNum.length>10){
						CMC.alertMessage("充值金额应为5的倍数，且不能小于3位数，或大于10位！", 'error');
						return ;
					}
					if(remark.length>100){
						CMC.alertMessage("备注不能大于100位！", 'error');
						return ;	
					}
					CMC.dialog('coupAccountPaymentSingaporeCreatFrom','close');
					CMC.request({
