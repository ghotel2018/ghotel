...
var CouponGroupCommon = {
    ...
    getConfigUrlI18n:"authorized/couponGroupI18n/pageConfig" ,
    getConfigUrlAUS:"authorized/couponGroupAUS/pageConfig" ,
    getConfigUrlKorea:"authorized/couponGroupKorea/pageConfig" ,
    getConfigUrlNZL:"authorized/couponGroupNZL/pageConfig" ,
    ...
    addKorea: "authorized/couponGroupKorea/addKorea" ,
    addNZL: "authorized/couponGroupKorea/addNZL" ,
    getUrl: "authorized/couponGroup/get/",
    checkDayDomesticUrl: "authorized/couponGroup/checkDay",
    checkDayI18nUrl: "authorized/couponGroupI18n/checkDay",
    checkDayAUSUrl: "authorized/couponGroupAUS/checkDay"
};
var accountSum;
var upFileFlag;
var issueFileFlag;
...
(function($){
    ...
    CouponGroupCommon.getDetail = function(formId, ind , groupId){
    	var update = document.getElementById('ismodify').value;
        $("#holdBtn_" + ind).show();
        $("#unlockBtn_" + ind).show();
...
//                $('#rateId_' + ind).val(bean['rateId']);
               ...
               $('#segPriceLimitSeat_' + ind).numberbox('setValue',bean['segPriceLimitSeat']);
               $('#segPriceLimitLuggage_' + ind).numberbox('setValue',bean['segPriceLimitLuggage']);
                if(bean['remark']){
        	    ...
        	    if(update=="update"){
            		...
            		if(jp == "jp"){
            			$("#segPriceLimit_update").textbox({disabled:false});
            			$("#segPriceLimitSeat_update").textbox({disabled:true});
            			$("#segPriceLimitLuggage_update").textbox({disabled:true});
            		} 
            		if(xz == "xz"){
            			$("#segPriceLimit_update").textbox({disabled:true});
            			$("#segPriceLimitSeat_update").textbox({disabled:false});
            			$("#segPriceLimitLuggage_update").textbox({disabled:true});
            		} 
            		if(xl == "xl"){
            			$("#segPriceLimit_update").textbox({disabled:true});
            			$("#segPriceLimitSeat_update").textbox({disabled:true});
            			$("#segPriceLimitLuggage_update").textbox({disabled:false});
            		} 
            		if(jp == "jp" && xz == "xz"){
            			$("#segPriceLimit_update").textbox({disabled:false});
            			$("#segPriceLimitSeat_update").textbox({disabled:false});
    ...
    $("#updateBtn_update").click(function(){
        //$('#rateId_update').val('');
        if(checkData('update','#updateCouponGroupForm')){//如果是运价优惠卷,则不要使用checkAccount()
            var data = CouponGroupCommon.handleGroupData('#updateCouponGroupForm','update');
            if(data==null || typeof data =='undefined'){
                return;
            }
            CouponGroupCommon.submitUpdateRequest(data,CouponGroupCommon.updateDomesticUrl,'update');
        }
    });
    ...
    CouponGroupCommon.handleGroupData = function(formId,ind){
        ...
        if(check){
            ...
            var segPriceLimitSeat = $.trim($("#segPriceLimitSeat_"+ind).numberbox('getValue'));
            var segPriceLimitLuggage = $.trim($("#segPriceLimitLuggage_"+ind).numberbox('getValue'));
            //航班选择时间
...
//            var rateId = $.trim($("#rateId_"+ind).val());//运价ID
            var rateId = $.trim($("#rateId_"+ind).textbox('getValue'));//运价ID
            //使用渠道
            var usedChannelStr = "";
            ...
            return  {
                ...
                "segPriceLimitSeat":segPriceLimitSeat,
                "segPriceLimitLuggage":segPriceLimitLuggage,
    ...
    CouponGroupCommon.init = function(scope){
    	$("#couponBusiness_updatejp").attr("disabled",true);
    	$("#couponBusiness_updatexz").attr("disabled",true);
    	$("#couponBusiness_updatexl").attr("disabled",true);
    	$("#seatRestriction_updatee").attr("disabled",true);
    	$("#seatRestriction_updateh").attr("disabled",true);
    	$("#seatRestriction_updateb").attr("disabled",true);
    	$("#seatRestriction_updateg").attr("disabled",true);
    	$("#seatRestriction_updatec").attr("disabled",true);
    ...
    CouponGroupCommon.intConfig = function(initChannel,initGateWay, initAccount,scopes){
    	var scope = scopes;
    	if (typeof(scope) == "undefined"){
    		if($("#isdomestic").val()=="I18n"){
    			scope = "I18n";
    		}else{
    			scope = "domestic";
    		}
    	}
    	$("#isdomestic").val(scope);
        var configUrl='';
        console.log(scope);
        if (scope=="I18n"){
            configUrl =CouponGroupCommon.getConfigUrlI18n;
        }else if (scope=="AUS"){
            configUrl =CouponGroupCommon.getConfigUrlAUS;
            $("#payGatewayList_foreign").html("");
        }else if(scope=="korea"){
            configUrl=CouponGroupCommon.getConfigUrlKorea;
        }else if(scope=="NZL"){
            configUrl=CouponGroupCommon.getConfigUrlNZL;
        }else{
            configUrl=CouponGroupCommon.getConfigUrl;
        }
        CMC.request({
            url: configUrl ,
            method: 'GET',
...
function CheckCouponBusiness(param){
	var CouponBusiness = document.getElementsByName('couponBusiness'); 
	var seatRestriction = document.getElementsByName("seatRestriction");
	var isfalg = false;
	...
	for(var i=0;i<CouponBusiness.length;i++){
		if(CouponBusiness[i].checked){
			if(CouponBusiness[i].value==1){
				isfalg = true;
				xz = "xz";
				$("#segPriceLimit_create").textbox({disabled:false});
				$("#segPriceLimitSeat_create").textbox({disabled:true});
				$("#segPriceLimitLuggage_create").textbox({disabled:true});
			}else if(CouponBusiness[i].value==2){
				xl = "xl";
				$("#segPriceLimit_create").textbox({disabled:true});
				$("#segPriceLimitSeat_create").textbox({disabled:false});
				$("#segPriceLimitLuggage_create").textbox({disabled:true});
			}else{
				jp = "jp";
				$("#segPriceLimit_create").textbox({disabled:true});
				$("#segPriceLimitSeat_create").textbox({disabled:true});
				$("#segPriceLimitLuggage_create").textbox({disabled:false});
			}
		}
	} 
	...
	if(jp == "jp"){
		...
		$("#segPriceLimitSeat_create").textbox({disabled:true});
		$("#segPriceLimitLuggage_create").textbox({disabled:true});
		$('#segPriceLimitSeat_create').textbox('clear');
		$('#segPriceLimitLuggage_create').textbox('clear');
	} 
	if(xz == "xz"){
		$("#segPriceLimit_create").textbox({disabled:true});
		$("#segPriceLimitSeat_create").textbox({disabled:false});
		$("#segPriceLimitLuggage_create").textbox({disabled:true});
		$('#segPriceLimit_create').textbox('clear');
		$('#segPriceLimitLuggage_create').textbox('clear');
	} 
	if(xl == "xl"){
		$("#segPriceLimit_create").textbox({disabled:true});
	...
	if(isfalg){
		for(i=0;i<seatRestriction.length;i++){
			seatRestriction[i].disabled=false;
		}
	}else{
		for(i=0;i<seatRestriction.length;i++){
			seatRestriction[i].disabled = true;
			seatRestriction[i].checked = false;
		}
	}
	if(param==1){
		var faceValueChecktwo = $.trim($("#faceValue_create").numberbox('getValue'));
		faceValueCheck(faceValueChecktwo,0);
	}
}

faceValueCheck = function(newv, oldv){
	var obj=document.getElementsByName('couponBusiness'); 
	var msg = "";
	var facevalue = $.trim(newv);
	var isfalg = false;
	...
	if(facevalue != ""){
		if(isfalg){
			if((!(/^[0-9]+$/g.test(facevalue))) || facevalue.length>3 || facevalue.length<1 || parseInt(facevalue)%10 != 0){
				msg = "机票产品的优惠券最小面值为10，10-990之间10的倍数!";
			}
		}else{
			if((!(/^[0-9]+$/g.test(facevalue))) || facevalue.length>3 || facevalue.length<1){
				msg = "增值产品的优惠券最小面值为1，1-999之间整数!";
			}
		}
	}else{
		msg = "请输入面值！";
	}
    $(".errorfaceValue").html(msg);
}
...
segPriceLimitSeatCheck = function(newv, oldv){
    var msg = "";
    var segPriceLimit = newv;
    if(segPriceLimit != ""){
        if(!(/^[0-9]+$/g.test(segPriceLimit))){
...
segPriceLimitLuggageCheck = function(newv, oldv){
    var msg = "";
    var segPriceLimit = newv;
    if(segPriceLimit != ""){
        if(!(/^[0-9]+$/g.test(segPriceLimit))){
...
discountTypeCheck = function(newv, oldv){
	if(!(document.getElementById("ismodify").value!="add")){
		if($("#discountType_create").combobox('getValue')==1){
			$("#couponBusiness_createjp").attr("checked",true);
			$("#couponBusiness_createxz").attr("disabled",true);
			$("#couponBusiness_createxl").attr("disabled",true);
			$("#couponBusiness_createxl").attr("checked",false);
			$("#couponBusiness_createxz").attr("checked",false);
			CheckCouponBusiness(1);
		}else{
//		$("#CouponBusiness_createjp").attr("checked",false);
			$("#couponBusiness_createxz").attr("disabled",false);//输入框可编辑
			$("#couponBusiness_createxl").attr("disabled",false);
		}
	}
...
//    $(this).parent().find("input[name='rateId']").val('');
    var update = document.getElementById('ismodify').value;
    if(update=="update"){
    	$('#rateId_update').textbox('setValue','');
    }else{
    	$('#rateId_create').textbox('setValue','');
    }
    if(typeValue == ""){
        msg = "请选择优惠卷类型!";
        $(this).parent().find(".error").html(msg);
...
function checkData(ind,formId){
    ...
    if($("#discountType_"+ind).combobox('getValue')=="1"&&$.trim($('#rateId_' + ind).textbox('getValue'))==""){
    ...
    if($(formId+ " input[name='couponBusiness']:checked").length==0){
        $(formId + " input[name='couponBusiness']:first").parent().children(".error").html("请选择优惠券适用业务！");
    }else{
        $(formId + " input[name='couponBusiness']:first").parent().children(".error").html("");
    }
	...
	if(xz == "xz"){
		if($("#segPriceLimitSeat_"+ind).numberbox('getValue')==""){
	        $("#segPriceLimitSeat_"+ind).parent().children(".error").html("请输入票价限制！");
	    }else{
	        $("#segPriceLimitSeat_"+ind).parent().children(".error").html("");
	    }
	} 
	if(xl == "xl"){
		if($("#segPriceLimitLuggage_"+ind).numberbox('getValue')==""){
	        $("#segPriceLimitLuggage_"+ind).parent().children(".error").html("请输入票价限制！");
	    }else{
	        $("#segPriceLimitLuggage_"+ind).parent().children(".error").html("");
	    }
	} 
	if(jp == "jp" && xz == "xz"){
		if($("#segPriceLimit_"+ind).numberbox('getValue')=="" || $("#segPriceLimitSeat_"+ind).numberbox('getValue')==""){
	        $("#segPriceLimit_"+ind).parent().children(".error").html("请输入票价限制！");
	    }else{
	        $("#segPriceLimit_"+ind).parent().children(".error").html("");
	    }
	...
	if(jp == "jp" && xl == "xl"){
		if($("#segPriceLimit_"+ind).numberbox('getValue')=="" || $("#segPriceLimitLuggage_"+ind).numberbox('getValue')==""){
	        $("#segPriceLimit_"+ind).parent().children(".error").html("请输入票价限制！");
	    }else{
	        $("#segPriceLimit_"+ind).parent().children(".error").html("");
	    }
	...
	if(xz == "xz" && xl == "xl"){
		if($("#segPriceLimitSeat_"+ind).numberbox('getValue')=="" || $("#segPriceLimitLuggage_"+ind).numberbox('getValue')==""){
	        $("#segPriceLimit_"+ind).parent().children(".error").html("请输入票价限制！");
	    }else{
	        $("#segPriceLimit_"+ind).parent().children(".error").html("");
	    }
	...
	if(jp == "jp" && xz == "xz" && xl == "xl"){
		if($("#segPriceLimit_"+ind).numberbox('getValue')=="" || $("#segPriceLimitSeat_"+ind).numberbox('getValue')=="" || $("#segPriceLimitLuggage_"+ind).numberbox('getValue')==""){
	        $("#segPriceLimit_"+ind).parent().children(".error").html("请输入票价限制！");
	    }else{
	        $("#segPriceLimit_"+ind).parent().children(".error").html("");
	    }
    ...
    $(formId ).find(".errorfaceValue").each(function(){
        if($(this).text()!=""){
            checkFlag = false;
            return;
        }
...
function statusmanage(formId,str,ind){
    ...
    var url =  CouponGroupCommon.updateDomesticUrl;
    if(ind =='update_I18n'){
...
function addI18n() {
    if(checkData('create','#createCouponGroupForm')){//如果是运价优惠卷,则不要使用checkAccount()
        ...
        CouponGroupCommon.submitAddRequest(data,CouponGroupCommon.addI18nUrl,'#createCouponGroupForm','create');
...
function addAUS() {
    if(checkData('create','#createCouponGroupForm')){//如果是运价优惠卷,则不要使用checkAccount()
        ...
        CouponGroupCommon.submitAddRequest(data,CouponGroupCommon.addAUSUrl,'#createCouponGroupForm','create','AUS');
...
function addKorea() {
    if(checkData('create','#createCouponGroupForm')){//如果是运价优惠卷,则不要使用checkAccount()
        ...
        CouponGroupCommon.submitAddRequest(data,CouponGroupCommon.addKorea,'#createCouponGroupForm','create');
...
function addNZL() {
    if(checkData('create','#createCouponGroupForm')){//如果是运价优惠卷,则不要使用checkAccount()
        ...
        CouponGroupCommon.submitAddRequest(data,CouponGroupCommon.addNZL,'#createCouponGroupForm','create','NZL');
...
function addDomestic() {
    if(checkData('create','#createCouponGroupForm')){//如果是运价优惠卷,则不要使用checkAccount()
        ...
        CMC.request({
            url: CouponGroupCommon.checkDayDomesticUrl ,
            method: 'post',
            async: false,
            data:data,
            success: function(message){
                if(message.statusCode==0){
                    CouponGroupCommon.submitAddRequest(data,CouponGroupCommon.addDomesticUrl,'create');
                }else{
