/**
 * Created by huangxin on 2017/7/14.
 */
var CouponGroupCommon = {
    menuId: "CouponGroupNZL",
    searchTableRequired: false,
    getConfigNZLUrl:"authorized/couponGroupNZL/pageConfig" ,
    ...
    checkDayNZLUrl: "authorized/couponGroupNZL/checkDay"
};
var accountSum;
var upFileFlag;
var issueFileFlag;
...
var updateactivityDescs;
var updateuseConditionDesc;
...
//	    readonly:true,
    ...
    CouponGroupCommon.getDetail = function(formId, ind , groupId){
        ...
        CMC.request({
            ...
            success : function(response) {
               ...
               /* $('#activityDesc_' + ind).textbox('setValue',bean['activityDesc']);
                $('#useConditionDesc_' + ind).textbox('setValue',bean['useConditionDesc']);*/
               ...
               /* if(bean['flightDate']){
                ...
                if(bean['isStriction'])
                	...
                	$("#userestriction_" + ind + "_NZL").combobox('setValue',bean['isStriction']);
    ...
    $("#updateBtn_update").click(function(){
        ...
        if(checkData('update','#updateCouponGroupForm')){//如果是运价优惠卷,则不要使用checkAccount()
            ...
            CouponGroupCommon.submitUpdateRequest(data,CouponGroupCommon.updateNZLUrl,'update');
        }
    });
    ...
    CouponGroupCommon.handleGroupData = function(formId,ind){
        ...
        if(check){
            ...
            var timeZoneSQ = $("#timeZoneSQ_"+ind).combobox('getText');
            var timeZoneSJ = $("#timeZoneSJ_"+ind).combobox('getText');
            var timeZone = timeZoneSQ +":"+ timeZoneSJ;
    ...
    CouponGroupCommon.intConfig = function(initChannel,initGateWay, initAccount,scope){
        ...
        var configUrl= CouponGroupCommon.getConfigNZLUrl;
        ...
        CMC.request({
            ...
            success: function(message){
                ...
                if(initGateWay){
                    ...
                    $("#gatewayTable_create_NZL").html(gatewayHtml);
                    $("#gatewayTable_update_NZL").html(gatewayHtml);
                }
                ...
                if(initAccount){
                    if(message.messageBody.virtualAccount){
                        ...
                        $("#accountValue_NZL").text(accountValue);
                        $("#update_accountValue_NZL").text(accountValue);
...
faceValueCheck = function(newv, oldv){
    ...
    if(facevalue != ""){
        if(!(/^[0-9]+$/g.test(facevalue))){
            ...
            if(facevalue.length>7){
                msg="输入的面值不能大于七位，请重新输入";
            }else{
            	msg = "";
            }
        }
...
function statusmanage(formId,str,ind){
    ...
    var url =  CouponGroupCommon.updateNZLUrl;
...
function addNZL() {
	if(isSegmentInfo){
		if(checkData('create','#createCouponGroupForm')){//如果是运价优惠卷,则不要使用checkAccount()
			...
			CouponGroupCommon.submitAddRequest(data,CouponGroupCommon.addNZLUrl,'#createCouponGroupForm','create','NZL');
			
		}
	}else{
		CMC.alertMessage("请填写正确的航班限制范围！",'warn');
	}
};
