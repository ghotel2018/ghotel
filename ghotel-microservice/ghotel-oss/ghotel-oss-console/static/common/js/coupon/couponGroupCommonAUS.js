...
var CouponGroupCommon = {
    ...
    getConfigAUSUrl:"authorized/couponGroupAUS/pageConfig" ,
...
//	    readonly:true,
        ...
        CMC.request({
            ...
            success : function(response) {
               ...
               /* if(bean['flightDate']){
                ...
                //$('#createChannel_' + ind).combobox({editable:false});
                ...
                if(bean['isStriction']){
                	...
                	$("#userestriction_" + ind + "_AUS").combobox('setValue',bean['isStriction']);
                }else{
                	$("#userestriction_" + ind).combobox('setValue',"1");
                }
    ...
    $("#updateBtn_update").click(function(){
        //$('#rateId_update').val('');
        if(checkData('update','#updateCouponGroupAUSForm')){//如果是运价优惠卷,则不要使用checkAccount()
            var data = CouponGroupCommon.handleGroupData('#updateCouponGroupAUSForm','update');
            if(data==null || typeof data =='undefined'){
                return;
            }
            CouponGroupCommon.submitUpdateRequest(data,CouponGroupCommon.updateAUSUrl,'update');
        }
    });
    ...
    CouponGroupCommon.handleGroupData = function(formId,ind){
        ...
        if(check){
            ...
            //duibiDateTime(flightDateOne_,flightDateTwo_,"flightDateOne_",ind);
            ...
            //duibiDateTime(usefulDateOne_,usefulDateTwo_,"usefulDateOne_",ind);
                    ...
                    success: function(response){
                        try{
                            ...
                            var groupId = getvl("groupId");
              			    CouponGroupCommon.getDetail("#updateCouponGroupAUSForm",'update',groupId);
                        }catch(e){
    ...
    CouponGroupCommon.intConfig = function(initChannel,initGateWay, initAccount,scope){
        ...
        var configUrl= CouponGroupCommon.getConfigAUSUrl;
        ...
        CMC.request({
            ...
            success: function(message){
                ...
                if(initGateWay){
                    ...
                    $("#gatewayTable_create_AUS").html(gatewayHtml);
                    $("#gatewayTable_update_AUS").html(gatewayHtml);
                }
                ...
                if(initAccount){
                    if(message.messageBody.virtualAccount){
                        ...
                        $("#accountValue_AUS").text(accountValue);
                        $("#update_accountValue_AUS").text(accountValue);
...
function statusmanage(formId,str,ind){
    ...
    var url =  CouponGroupCommon.updateAUSUrl;
...
/*function duibiDateTime(startDate,endDate,info,type){
	var startDateNum = Date.parse(startDate);
	var endDateNum = Date.parse(endDate);
	if(startDateNum > endDateNum ){
        $("#"+info+type).parent().find('.error').html('开始日期不能大于结束日期！');
        return
    }else{
        $("#"+info+type).parent().find('.error').html('');
    }
}*/