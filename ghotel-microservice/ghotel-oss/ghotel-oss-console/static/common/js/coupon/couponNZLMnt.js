...
var CouponMnt = {
		...
		searchUrl: "authorized/couponNZL/getAll" , //如果searchTableRequired 是 true 必填
		getUrl: "authorized/couponNZL/get/" ,
		batchUpdateUrl: "authorized/couponNZL/batchUpdate" ,
		exportUrl: "authorized/couponNZL/export" ,
		getConfigUrl : "authorized/couponNZL/getConfig" ,
		initNotSearch:true,
		columns : [ [  {
			field : 'groupId',
			title : '所属批次号',
...
(function($) {
	...
	$("#couponNZLSearch").click(function(){
	...
	$('#createReportingTask').click(function(){
		...
		/*if(groupId==""){
			CMC.alertMessage("请输入优惠劵批次!", 'warn');
			return;
		}*/
		CMC.request({
	...
	$("#checkCouponDetail").click(function(){
		...
		if (val == null && val != "") {
			...
			CMC.request({
						url : "/cmc/authorized/couponNZL/get/"+val['groupId'],
						type : "get",
						dataType : "json",
						async : true,
						success : function(response) {
							...
							if (response.messageBody){
								...
								$("#codeInfo_table #userestriction_nzl").combobox('setValue',detailInfo['isStriction']);
								...
								console.log(detailInfo["cabinType"]);
