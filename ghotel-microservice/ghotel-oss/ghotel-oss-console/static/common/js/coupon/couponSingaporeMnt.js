...
var CouponSingaporeMnt = {
		menuId : 'CouponSingapore',
		searchTableRequired : true,
		initNotSearch:true,
		searchTableWidth: '100%',
		searchUrl: "authorized/couponSingapore/getAll" , //如果searchTableRequired 是 true 必填
		getUrl: "authorized/couponSingapore/get/" ,
		batchUpdateUrl: "authorized/couponSingapore/batchUpdate" ,
		exportUrl: "authorized/couponSingapore/export" ,
		getConfigUrl : "authorized/couponSingapore/getConfig" ,
		columns : [ [  {
			field : 'groupId',
			title : '所属批次号',
			width : '15%',
...
(function($) {
	...
	$("#couponSingaporeSearch").click(function(){
	...
	$('#createReportingTask').click(function(){
		...
		if(startUserDate!=""&&endUserDate!=""){
			if(!CouponSingaporeMnt.isArea(startUserDate,endUserDate)){
				return;
			}
		}
		...
		if(createDateStart!=""&&createDateEnd!=""){
			if(!CouponSingaporeMnt.isArea(createDateStart,createDateEnd)){
				return;
			}
		}
		...
		if(useDateStart!=""&&useDateEnd!=""){
			if(!CouponSingaporeMnt.isArea(useDateStart,useDateEnd)){
				return;
			}
		}
		...
		CMC.request({
			url: CouponSingaporeMnt.exportUrl,
			method: 'POST',
	...
	$("#checkCouponDetail").click(function(){
		...
		if (val == null && val != "") {
			...
			CMC.request({
						url : "/cmc/authorized/couponSingapore/get/"+val['groupId'],
						type : "get",
						dataType : "json",
						async : true,
						success : function(response) {
							...
							if (response.messageBody){
								...
								$("#codeInfo_table #userestriction_Singapore").combobox('setValue',detailInfo['isStriction']);
	...
	CouponSingaporeMnt.showDetails = function(info){
	...
	CouponSingaporeMnt.init = function(){
		CouponSingaporeMnt.intConfig();
	}
	
	CouponSingaporeMnt.intConfig = function(){
		
		CMC.request({
			url: CouponSingaporeMnt.getConfigUrl ,
			method: 'GET',
			success: function(message){
	...
	$('#reportDataClose').click(function(){
		var reportData ="";
	...
	$('#showReportExportDialog').click(function(){
		...
		$("#groupIds").textbox('setValue','');
		$("#reportDate_value").val("");
		$("INPUT[name='report']:checkbox").prop("checked", true).attr("checked", false);
	...
	$("#batchUpdateSubmit").click(function(){
		...
		CMC.showProcessBox();
		CMC.fileUpload({
			url: CouponSingaporeMnt.batchUpdateUrl,
			method: "POST",
			dataType: "json",
	 ...
	 * 判断时间范围
	 */
	CouponSingaporeMnt.isArea=function(start,end){
		var startD;
		var endD;
		var days;
