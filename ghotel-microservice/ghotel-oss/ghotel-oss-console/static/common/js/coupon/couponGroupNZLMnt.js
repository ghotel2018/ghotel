...
var CouponGroupMnt = {
	scope:'NZL',
	menuId: "CouponGroupNZL",
	searchTableRequired: true,
	getConfigUrl: "authorized/couponGroupNZL/pageConfig" ,
	addDomesticUrl: "authorized/couponGroupNZL/addDomestic" ,
	updateDomesticUrl: "authorized/couponGroupNZL/updateDomestic" ,
	addNZLUrl: "authorized/couponGroupNZL/addNZL" ,
	updateNZLUrl: "authorized/couponGroupNZL/updateNZL" ,
	getUrl: "authorized/couponGroupNZL/get/" ,
	issueUrl: "authorized/couponGroupNZL/issue" ,
	exportKeyUrl: "authorized/couponGroupNZL/exportPrivateKey" ,
	exportReportUrl: "authorized/couponGroupNZL/exportReport",
	getMailSentCountUrl: "authorized/couponGroupNZL/getMailSentCount",
	updateMailSentCountUrl: "authorized/couponGroupNZL/updateMailSentCount",
	searchUrl: "authorized/couponGroupNZL/getNZLAll" , //如果searchTableRequired 是 true 必填
	searchNZLUrl: "authorized/couponGroupNZL/getNZLAll" , //如果searchTableRequired 是 true 必填
	...
	columns :  [[
		...
		{field: 'discountType', title:'优惠券类型' , width: '20%' , align: 'center',
			formatter:function(value,row,index){
				   var discountType="";
				   if(row.discountType==13||row.discountType=='13'){
					   discountType='新西兰自有资金优惠券';
				   }else if(row.discountType==14||row.discountType=='14'){
					   discountType='新西兰虚拟资金优惠券';
				   }
				   return discountType;
			   }
		},
...
(function($){
	...
	$('#searchFormAddNZL').click(function(){
		...
		$("#createDialog").dialog({
			...
			cache:false,
			modal:true,
			href:"/cmc/module/coupon/couponGroupCreateNZLForm.html?scope=NZL",
			onLoad:function () {

				$("#createDialog #isInternations_create").val("4");
				$("#createDialog #addBtn_create_domestic").hide();
				$("#createDialog #addBtn_create_NZL").show();
				...
				CouponGroupCommon.intConfig(true,true,true,'NZL');
			}
			//href:"/cmc/module/coupon/couponGroupCreateForm.html?scope=AUS",

		});
		//$("#showChannelName_AUS").val();
		//$("#showChannelName_AUS").hide();
		//$('#createAUSCouponGroupForm').form('clear');
		$('.error').html('');
		$("#count").html(500);
		//$("#addBtn_create_AUS").show(); //
		//$('#isInternations_create_AUS').val('1');
		$('#segmentType_create_NZL').combobox('setValue','0');
		$("#createNZLCouponGroupForm input[name='flightTimeType']").each(function(j,el2){
			$(el2).prop("checked", true).attr("checked", true);
		});

		$("#createNZLCouponGroupForm .easyui-datebox").each(function(j,el2){
			$(el2).datebox().datebox('calendar').calendar({
				validator: function(date){
					var now = new Date();
					var d1 = new Date(now.getFullYear(), now.getMonth(), now.getDate());
	...
	$('#searchFormUpdateNZL').click(function(){
		var record = CMC.grid.datagrid("getSelected");
		if(!record || record['groupId']==null || record['groupId']==''){
			CMC.alertMessage("请选择一条新西兰优惠券批次记录!", 'warn');
			return;
		}
		CouponGroupMnt.intConfig(false,false,true);
		var groupId = record['groupId'];
		var src = "/cmc/module/coupon/couponGroupUpdateNZLForm.html?scope=NZL&groupId="+groupId;
		//CouponGroupMnt.intConfig(false,false,true);

		$('.error').html('');
		$('#updateNZLCouponGroupForm').form('clear');
		...
		//CouponGroupMnt.getDetail("#updateCouponForm",'update_AUS',groupId);
	});
	...
	$("#searchFormIssue").click(function(){
		...
		if($("#issueFormDialog #container").length<=0){
			...
			$('#issueFormDialog').show().dialog({
				...
				cache: true,
				href:"/cmc/module/coupon/couponIssuNZLForm.html"
			});
		}else{
...
//		CMC.dialog('winMarketingActivityEffect','open');
	...
	$("#CouponGroup_Template").click(function(){
		window.open(encodeURI("/cmc/download/couponGroupNZLTemplate.xlsx"));
	});
	...
	$("#CouponGroupInfo_import").click(function(){
		...
		CMC.confirm("是否确认导入文件?",function(r){
			if(r){
				 CMC.showProcessBox();
				CMC.fileUpload({
					url: "authorized/couponGroupNZL/importReport",
					type: "POST",
					dataType: "json",
