var CouponGroupMnt = {
		menuId: "CouponGroupOverseas",
		searchTableRequired: true,
		getConfigUrl: "authorized/couponGroupOverseas/pageConfig" , 
		addOverseasUrl: "authorized/couponGroupOverseas/addOverseas" , 
		updateOverseasUrl: "authorized/couponGroupOverseas/updateOverseas" , 
		getUrl: "authorized/couponGroupOverseas/get/" , 
		issueUrl: "authorized/couponGroupOverseas/issue" , 
		exportKeyUrl: "authorized/couponGroupOverseas/exportPrivateKey" ,
		getMailSentCountUrl: "authorized/couponGroupOverseas/getMailSentCount",
		updateMailSentCountUrl: "authorized/couponGroupOverseas/updateMailSentCount",
//		exportReportUrl: "authorized/couponGroupOverseas/exportReport",
		searchUrl: "authorized/couponGroupOverseas/getOverseasAll" , //如果searchTableRequired 是 true 必填
		columns :  [[
			           ...
			           {field: 'discountType', title:'优惠券类型' , width: '20%' , align: 'center',
						   formatter:function(value,row,index){
							   var discountType="";
							   if(row.discountType==17||row.discountType=='17'){
								   discountType='英国自有资金优惠券';
							   }else if(row.discountType==18||row.discountType=='18'){
								   discountType='英国虚拟资金优惠券';
							   }
							   return discountType;
						   } 
					   },
...
$(document).ready(function(){
	...
	if(isInternations=="6"||isInternations==6){
		channel ="161885";
		param = "?channel=161885&isInternations="+isInternations;
		currency = "GBP";
	}
...
(function($){
	...
	$("#searchFormAddOverseas").click(function(){
		$("#createDialog").dialog({
			title:'优惠券生成',
			closed:false,
			cache:false,
			modal:true,
			href:"/cmc/module/coupon/couponGroupOverseasCreateForm.html"+param,
			onLoad:function () {
				...
				$("#createDialog #addBtn_create_Overseas").show();
				...
				CouponGroupCommon.intConfig(true,true,true,'Overseas');
	...
	$('#searchFormUpdateOverseas').click(function(){
		var record = CMC.grid.datagrid("getSelected");
		if(!record ){
			CMC.alertMessage("请选择一条英国优惠券批次记录!", 'warn');
			return;
		}
		CouponGroupMnt.intConfig(false,false,true);
		var groupId = record['groupId'];
		var src = "/cmc/module/coupon/couponGroupOverseasUpdateForm.html"+param+"&groupId="+groupId;
		//CouponGroupMnt.intConfig(false,false,true);
		$('.error').html('');
	...
	$("#searchFormIssue").click(function(){
		...
		if($("#issueFormDialog #container").length<=0){
			...
			$('#issueFormDialog').show().dialog({
				...
				left:winTopLeft.winLeft,
				cache: true,
				href:"/cmc/module/coupon/couponIssuOverseasForm.html"
			});
		}else{
...
//		CMC.dialog('winMarketingActivityEffect','open');
	...
	$("#CouponGroup_Template").click(function(){
		window.open(encodeURI("/cmc/download/couponGroupOverseasTemplate.xlsx"));
	});
	/**
	 * 执行文件导入
	 */
	$("#CouponGroupInfo_import").click(function(){
		...
		CMC.confirm("是否确认导入文件?",function(r){
			if(r){
				 CMC.showProcessBox();
				CMC.fileUpload({
					url: "authorized/couponGroupOverseas/importReport" + param,
					type: "POST",
					dataType: "json",
